...
learning_rates = [0.0001, 0.001, 0.01, 0.02, 0.05, 0.1]
...
for lr in learning_rates:
    ...
    model.add(Dense(420, input_dim=X_train.shape[1], activation='relu', kernel_initializer=HeNormal()))
    model.add(BatchNormalization())
    model.add(Dropout(0.2))

    # Second hidden layer
    model.add(Dense(420, activation='relu', kernel_initializer=GlorotUniform()))
    model.add(BatchNormalization())
    model.add(Dropout(0.2))
    ...
    print(f'Learning rate: {lr}, MSE: {mse}, R-squared: {r2}')
